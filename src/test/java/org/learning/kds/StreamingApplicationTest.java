...
public class StreamingApplicationTest {
    ...
    public void shouldRun(
            @InjectClusterClient ClusterClient<String> flinkClusterClient,
        ...
        var application = new StreamingApplication(sourceConfig);

        Metrics metrics = Metrics.builder()
                .unit("percent")
                .build();

        kinesisManager.publishEvent(metrics);

        var jobId = application.run();

        Thread.sleep(3000);
        var stopRequest = flinkClusterClient.stopWithSavepoint(
                jobId,
                true,
